...
func TestConfig_NewConfig_env(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			want:    Config{Host: "localhost:8080", StoreInterval: 100, FileStoragePath: "backup.dat", Restore: true},
			errWant: false,
		},
		{
		...
		},
		{
			...
			want:    Config{Host: "localhost:8080", StoreInterval: 300, FileStoragePath: "file.dat", Restore: true},
			errWant: false,
		},
		{
			name:    "Valid restore",
			env:     map[string]string{"RESTORE": "false"},
			want:    Config{Host: "localhost:8080", StoreInterval: 300, FileStoragePath: "backup.dat", Restore: false},
			errWant: false,
		},
	}
...
func TestConfig_NewConfig_Flags(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:    "Valid config",
			args:    []string{},
			want:    Config{Host: "localhost:8080", StoreInterval: 300, FileStoragePath: "backup.dat", Restore: true},
			errWant: false,
		},
		{
			name:    "Valid config",
